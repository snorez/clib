...
sudo chroot $DIR /bin/bash -c "useradd -m test -s /bin/bash -G sudo"
sudo chroot $DIR /bin/bash -c "passwd test -d"

sudo mkdir -p $DIR/root/.ssh/
sudo mkdir -p $DIR/home/test/.ssh/
cat $RELEASE.id_rsa.pub | sudo tee $DIR/root/.ssh/authorized_keys
cat $RELEASE.id_rsa.pub | sudo tee $DIR/home/test/.ssh/authorized_keys
