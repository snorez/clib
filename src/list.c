...
int list_comm_type_new_append(list_comm *head, void *new, size_t len)
{
	list_comm *new_node = (list_comm *)malloc(sizeof(list_comm)+len);
	if (!new_node) {
		err_dbg(0, err_fmt("malloc err"));
		errno = ENOMEM;
		return -1;
	}
	memset(new_node, 0, sizeof(list_comm)+len);
	memcpy((char *)new_node->extra, (char *)new, len);
	list_comm_append(head, new_node);
	return 0;
}

void list_comm_type_make_empty(list_comm *head,
			       list_comm_type_clean_func *callback)
{
	list_comm *prev = head, *next = head;
	list_for_each_entry(next, &head->list_head, list_head) {
		if (callback)
			callback((void *)next->extra);
