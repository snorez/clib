...
elf_file *do_elf_file(char *path)
{
	...
	ef->file = reg_file_open(path, O_RDONLY);
	if (!ef->file) {
		err_dbg(1, err_fmt("reg_file_open err"));
		err = -1;
		goto free_ret;
	}

	err = reg_file_readlines(ef->file);
	if (err == -1) {
	...
	list_comm *tmp = (list_comm *)(ef->file->pri_data.list_head.next);
